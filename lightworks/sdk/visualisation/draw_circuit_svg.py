...
class DrawCircuitSVG:
    ...
    def __init__(self, circuit: "Circuit",  # type: ignore
        ...
        if mode_labels is not None:
            ...
            mode_labels = [str(i)
                           for i in range(self.N - len(self.herald_modes))]
        ...
        self._add_heralds(
            self.circuit._external_heralds,
            self._init_length + 50,
            maxloc)
    ...
    def _add_ps(self, mode: int, phi: float) -> None:
        ...
        self.draw_spec += [("text", ("PS", xloc + size / 2,
                            yloc + 2, 0, 25, "white", "centred"))]
    ...
    def _add_unitary(self, mode1: int, mode2: int, label: str) -> None:
        ...
        self.draw_spec += [("unitary", (xloc, yloc,
                            size_x, size_y, offset / 2))]
    ...
    def _add_loss(self, mode: int, loss: float | str) -> None:
        ...
        self.draw_spec += [("text", ("L", xloc + size / 2,
                            yloc + 2, 0, 25, "white", "centred"))]
        # Add loss label
    ...
    def _add_grouped_circuit(self, mode1: int, mode2: int, name: str,
        ...
        for i in modes:
            if i not in self.herald_modes:
                ...
                self._add_wg(xloc + extra_length,
                             self.y_locations[i], con_length)
