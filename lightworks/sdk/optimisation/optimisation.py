...
class Optimisation:
    ...
    def _process(self) -> SamplingResult | SimulationResult:
        ...
        if self.__processor == "sampler":
            ...
            return sampler.sample_N_inputs(
                ...
                herald=self.__processor_args["herald"],  # type: ignore
                post_select=(
                    self.__processor_args["post_select"])  # type: ignore
            )
            # fmt: on
        else:
    ...
    def _test_opt_func(self) -> None:
        ...
        results = SamplingResult({State([0] * n): 0.5,
                                  State([1] + [0] * (n - 1)): 0.5},
                                 input=State([1] + [0] * (n - 1)))
        try:
